...
const GetEmployee = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/employee/${searchName}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div
      ...
      style={{
        background: "#121212", // 🔧 Changed to dark background
    ...
    >
      ...
      <div className="w-75 mb-4">
        <input
          type="text"
          className="form-control bg-dark text-light border-secondary" // 🔧 Dark input
        ...
        />
        <button
          className="btn btn-outline-light mt-2 w-100" // 🔧 Light outline button
      ...
      {/* 📋 Employee Table */}
      <div
        className="p-4 rounded shadow w-75"
        style={{ backgroundColor: "#1e1e1e" }} // 🔧 Dark card background
      >
        <h2 className="mb-4 text-center text-light">Employee List</h2>
        <table className="table table-dark table-hover table-bordered"> {/* 🔧 Dark table */}
            ...
            {employees.length === 0 ? (
              ...
              employees.map((employee) => (
                <tr key={employee.empId}>
                  ...
                  <td
                    style={{ color: "#4dabf7", cursor: "pointer" }} // 🔧 Light blue link
